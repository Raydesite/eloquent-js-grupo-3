...
function contarPor(elementos, nombreGrupo) {
  ...
  for (let elemento of elementos) {
    let res = nombreGrupo(elemento);
    let conocido = cuentas.findIndex((c) => c.nombre == res.nombre);
    if (conocido == -1) {
      cuentas.push({ ...res, cuenta: 1 });
    } else {
...
function direcciónDominante(texto) {
  let codigos = contarPor(texto, (caracter) => {
    let codigo = codigoCaracter(caracter.codePointAt(0));
    return codigo ? { nombre: codigo.name, direccion: codigo.direction } : 'no';
  }).filter((code) => code.name != 'no');

  if (!codigos.length) return 'No hay caracteres';

  return codigos.reduce((acc, actual) => {
    return actual.cuenta > acc.cuenta ? actual : acc;
  }).direccion;
}

console.log(direcciónDominante('Hello!')); // → ltr
console.log(direcciónDominante('Hey, مساء الخير')); // → rtl
console.log(direcciónDominante('英国的狗说"woof')); // ltr
console.log(direcciónDominante('俄罗斯的狗说"тяв"')); // ltr
console.log(direcciónDominante('Бүгдээрээ сайн байцгаана уу.')); // ltr
