...
/* Tu propio ciclo */
ciclo = (vi, prueba, actualiza, accion)=>{
    for(i=vi; prueba(i); i=actualiza(i)){
        accion(i);
    }
}
ciclo(1, i=>i<3, i=>i+1, console.log)
// -> 1
// -> 2
...
let direccionDom = (texto)=>{
    ...
    let mayor = porDirecc.reduce(
        (mayor, i) => mayor.cuenta > i.cuenta ? mayor:i
		)
    return mayor.nombre;
}
direccionDom('Test')
