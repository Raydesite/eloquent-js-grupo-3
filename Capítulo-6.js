...
class Group {
    ...
    add(e){
        if(!this.conjunto.includes(e))
            this.conjunto.push(e);
    }

    delete(e){
        let index = this.conjunto.indexOf(e);
        if( index >= 0)
          this.conjunto.splice(index, 1)
    }
...
class GroupIterator {
    ...
    next(){
        if(this.index == this.conjunto.conjunto.length)
            return {done: true}
...
hasOwnPropertySymbol = Symbol("hasOwnProperty")
